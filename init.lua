...
function Init:GlobalInit()
    ...
    PluginAPI.RegisterRoomEvent("NewAWActivation")
    ...
    local customRooms_bundle = CS.ScriptHelper.LoadBundle("customrooms")
    if customRooms_bundle then
        local customRoom_obj = customRooms_bundle:LoadAsset("LC_Room4-2.prefab", typeof(GameObject))
        if customRoom_obj then
            PluginAPI.RegisterRoom("LC_Room4-2", customRoom_obj, customRoom_obj)
        end
    end
end
...
function Init:InitServer()
    ...
    if getConfigBool("updated_rooms", false) then        
        PluginAPI.AddPreMapGenerationCallback(function(zones)            
            ...
            --Setting Lockroom and EZ_Medibay and removing EZ_Servers, EZ_Toilets, EZ_Poffices, EZ_Room3Offices, EZ_Sroom
            local ezone = {}
            for i = 0, zones[2].rooms.Length - 1 do
                ...
                if room.roomName ~= "EZ_Servers" and room.roomName ~= "EZ_Toilets" and room.roomName ~= "EZ_Poffices"
                and room.roomName ~= "EZ_Room3Offices" and room.roomName ~= "EZ_Sroom" then
            ...
            --Removing LC_Archive
            local lczone = {}
            for i = 0, zones[0].rooms.Length - 1 do
                local room = zones[0].rooms[i]
                if room.roomName ~= "LC_Archive" then
                    table.insert(lczone, room)
                end
            end
            zones[0].rooms = lczone
    ...
    CS.HookManager.Add("onRoundStart", function(obj)
        ...
        PluginAPI.SpawnNetworkedEvent("NewAWActivation")
