...
local function MapGeneratorCallback(zones)
    ...
    local lcz_4rooms_count = Config.GetInt("lcz_4rooms_count", 3)
    local hcz_4rooms_count = Config.GetInt("hcz_4rooms_count", 5)
    local ez_4rooms_count = Config.GetInt("ez_4rooms_count", 5)

    local lcz_2crooms_count = Config.GetInt("lcz_2crooms_count", 6)
    local hcz_2crooms_count = Config.GetInt("hcz_2crooms_count", 10)
    local ez_2crooms_count = Config.GetInt("ez_2crooms_count", 10)
...
function Init:InitServer()
    ...
    CS.HookManager.Add("startRound", function(obj)
        if GameObject.FindObjectsOfType(typeof(CS.Player)).Length < CS.Config.GetInt("start_round_minimum_players", 2) + 2 then
            --CS.Config.SetConfig("server_forced_class", 2)
